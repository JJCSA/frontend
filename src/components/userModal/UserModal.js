...
const UserModal = (props) => {
  ...
  const getCommunityProof = async (e) => {
    e.preventDefault();
    const response = await comm.get(`/admin/users/${props.data.id}/communityProof`, props.token, null);
      ...
      <Container fluid>
        <Row>
          <Col md={2} className="pl-0 pr-0 pt-1">
            <ImageFormatter cell={props.data.profilePicture} avatarSize="large" />
          </Col>
          ...
          <div className="info-container mt-1 rounded mb-3">
            ...
            <div className="divOutside last">
              <div className="mt-3 ml-2 mb-3">
                ...
                <Button variant="outline-secondary" className="ml-4" onClick={getCommunityProof}>
            ...
            <>
              <Row>
                <div className="card border-0 info-container">
                  <ToggleButtonGroup type="radio" name="options" value={status} onChange={changeStatus}>
                  ...
                  <div>
                    {status === Constants.userStatus.REJECTED
                      && <input type="text" value={rejectReason} className="form-control mt-2" placeholder="Reason for rejecting" onChange={(e) => setRejectReason(e.target.value)} required />}
                    {pendingAction && <Button type="submit" variant="outline-primary" className="mt-1, Btn-submit" onClick={submitStatusUpdate}>Submit Updates</Button>}
                  </div>
                </div>
              </Row>
            </>
          ...
          : (
            <>
              <Row>
                <Col md={6} style={{ paddingLeft: '0px' }}>
                  <div className="info-container rounded mb-3">
                    ...
                    <div className="divOutside">
                      <div className="mt-3 ml-2 mb-3">
                        {props.data.workExperience.map((experience_row, index) => (
                          <CareerInfo key={index} careerType="Experience" careerName={experience_row.company_name} careerDescription={experience_row.role} careerStart={experience_row.start_date} careerEnd={experience_row.end_date} />
                        ))}
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="divOutside">
                      <div className="mt-3 ml-2 mb-3">
                        {props.data.educationList.map((education_row, index) => (
                          <CareerInfo key={index} careerType="Education" careerName={education_row.universityName} careerDescription={education_row.degree} careerStart={education_row.start_date} careerEnd={education_row.end_date} />
                        ))}
                      </div>
                    </div>
                  </div>
              ...
              {props.data.userStatus === Constants.userStatus.ACTIVE &&
                ...
                globalState.globalState.profile.userRole === Constants.userTypes.SUPERADMIN && (
                  <Row>
                    <div className="info-container mt-1 rounded mb-3">
                      <div className="divOutside">
                        <div className="mt-3 ml-2 mb-3">
